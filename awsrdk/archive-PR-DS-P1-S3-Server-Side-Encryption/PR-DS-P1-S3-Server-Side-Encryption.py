...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    # print('Show configuration items dictionary:')
    # print(configuration_item)
    #
    # print('Show event:')
    # print(event)
    #
    # print('Show valid_rule_parameters :')
    # print(valid_rule_parameters)



    # Match on s3 bucket type
    if configuration_item['resourceType'] != 'AWS::S3::Bucket':
        # return {
        #     "compliance_type": "NOT_APPLICABLE",
        #     "annotation": "Only AWS::S3::Bucket types are applicable."
        # }
        annotation = "Only AWS::S3::Bucket types are applicable."
        return build_evaluation_from_config_item(configuration_item, 'NOT_APPLICABLE', annotation)

    if configuration_item['configurationItemStatus'] == "ResourceDeleted":
        annotation = "ResourceDeleted status AWS::S3::Bucket are not applicable."
        return build_evaluation_from_config_item(configuration_item, 'NOT_APPLICABLE', annotation)

    # Test for the approved exception cases that are supplied as a list to this rule
    if 'infosec-approved-exceptions' in valid_rule_parameters:

        if valid_rule_parameters:
            if valid_rule_parameters['infosec-approved-exceptions']:

                # Check list to compare bucket name to allowed exceptions
                if configuration_item['resourceName'] in valid_rule_parameters['infosec-approved-exceptions']:
                    annotation ='infosec-approved-exception to PR-DS-P1-S3-Server-Side-Encryption for ' + configuration_item['resourceName']
                    return build_evaluation_from_config_item(configuration_item, 'COMPLIANT', annotation)
    ...
    try:
        ...
        # print("A successful api call means we found default encryption.")
        # print(response)
        annotation ='Default encryption is set.'
        return build_evaluation_from_config_item(configuration_item, 'COMPLIANT', annotation)


    except:
        # If we receive an error, the default encryption flag is not set
        annotation ='The default encryption flag is not set.'
        return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', annotation)

# {'relatedEvents': [], 'relationships': [], 'configuration': {'name': 'ssarver-rdk-testing2', 'owner': {'displayName': None, 'id': '0cda69d020d896a314847e58034dcdcfe1dc00d5acf2b2547ee913111f65b367'}, 'creationDate': '2021-11-05T18:59:27.000Z'}, 'supplementaryConfiguration': {'AccessControlList': '{"grantSet":null,"grantList":[{"grantee":{"id":"0cda69d020d896a314847e58034dcdcfe1dc00d5acf2b2547ee913111f65b367","displayName":null},"permission":"FullControl"}],"owner":{"displayName":null,"id":"0cda69d020d896a314847e58034dcdcfe1dc00d5acf2b2547ee913111f65b367"},"isRequesterCharged":false}', 'BucketAccelerateConfiguration': {'status': None}, 'BucketLoggingConfiguration': {'destinationBucketName': None, 'logFilePrefix': None}, 'BucketNotificationConfiguration': {'configurations': {}}, 'BucketPolicy': {'policyText': None}, 'BucketVersioningConfiguration': {'status': 'Off', 'isMfaDeleteEnabled': None}, 'IsRequesterPaysEnabled': False, 'PublicAccessBlockConfiguration': {'blockPublicAcls': True, 'ignorePublicAcls': True, 'blockPublicPolicy': True, 'restrictPublicBuckets': True}}, 'tags': {}, 'configurationItemVersion': '1.3', 'configurationItemCaptureTime': '2021-11-05T19:00:23.554Z', 'configurationStateId': 1636138823554, 'awsAccountId': '223088857621', 'configurationItemStatus': 'ResourceDiscovered', 'resourceType': 'AWS::S3::Bucket', 'resourceId': 'ssarver-rdk-testing2', 'resourceName': 'ssarver-rdk-testing2', 'ARN': 'arn:aws:s3:::ssarver-rdk-testing2', 'awsRegion': 'us-east-1', 'availabilityZone': 'Regional', 'configurationStateMd5Hash': '', 'resourceCreationTime': '2021-11-05T18:59:27.000Z'}


    # Test nfor the specific property since it is an 3 Bucket and it is not and approved exception
    # if configuration_item['resourceType'][] != 'AWS::S3::Bucket'
