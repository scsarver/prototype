...
readonly REQUIRED_SOFTWARE='shellcheck aws jq yq'
...
TEST_DIRECTORY="$(date +%Y%m%d%H%M%S)-test"
TEST_FILES=(
one.yaml
two.yaml
three.yaml
...
echo "Create test directory: $(pwd)/$TEST_DIRECTORY"
cd "$SCRIPTDIR"
mkdir -p "$TEST_DIRECTORY"
cd "$SCRIPTDIR/$TEST_DIRECTORY"
...
# for FILE in $(ls | grep -E 'yaml|yml')
for FILE in *.yaml *.yml
do
  echo "$FILE"
done

echo "Remove test directory: $(pwd)/$TEST_DIRECTORY"
cd "$SCRIPTDIR"
